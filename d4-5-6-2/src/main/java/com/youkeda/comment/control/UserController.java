...
public class UserController {
    ...
    private static final TypeReference<List<UserDO>> t_pr = new TypeReference<List<UserDO>>() {
    };
    ...
    @PostConstruct
    public void init() {
        String content = getContent("classpath:datas/user.json");
        List<UserDO> users = JSON.parseObject(content, t_pr);
        ...
        for (UserDO userDO : users) {
            ...
            // 默认密码 123
            userDO.setPwd(DigestUtils.md5Hex("123" + "_ykd2050").toUpperCase());
            delete(id);
    ...
    public UserDO save(@RequestBody UserDO userDO) {
        ...
        redisTemplate.opsForHash().put(UserService.CACHE_KEY, userDO.getUserName(), userDO);
        return userDO;
    }
    ...
    public boolean delete(@RequestParam("id") Long id) {
        ...
        if (!CollectionUtils.isEmpty(users)) {
            ...
            redisTemplate.opsForHash().delete(UserService.CACHE_KEY, userDO.getUserName());
        }
    ...
    public UserDO findByUserName(@RequestParam("userName") String userName) {
        ...
        UserDO userDO = (UserDO)redisTemplate.opsForHash().get(UserService.CACHE_KEY, userName);
